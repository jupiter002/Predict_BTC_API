...
scaler_x.fit(train_set.loc[:, ['open','high','low','volume']])

# train_set.iloc[:, :-1] = scaler_x.transform(train_set.iloc[:, :-1])
# test_set.iloc[:, :-1] = scaler_x.transform(test_set.iloc[:, :-1])
train_set.loc[:, ['open','high','low','volume']] = scaler_x.transform(train_set.loc[:, ['open','high','low','volume']])
test_set.loc[:, ['open','high','low','volume']] = scaler_x.transform(test_set.loc[:, ['open','high','low','volume']])

# Output scale
scaler_y = MinMaxScaler()
scaler_y.fit(train_set.loc[:, ['close']])

# train_set.iloc[:, -1] = scaler_y.transform(train_set.iloc[:, [-1]])
# test_set.iloc[:, -1] = scaler_y.transform(test_set.iloc[:, [-1]])
train_set.loc[:, ['close']] = scaler_y.transform(train_set.loc[:, ['close']])
test_set.loc[:, ['close']] = scaler_y.transform(test_set.loc[:, ['close']])
...
def build_dataset(time_series, seq_length):
    ...
    for i in range(0, len(time_series)-seq_length):
        ...
        _x = time_series[i:i+seq_length, [0,1,2,4]]
